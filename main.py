...
class HlavnaScreen(MDScreen):
    ...
    def copy_full_shift_summary(self):
        ...
        # --- ŠTART NOVÉHO FORMÁTOVANIA HLÁSENÍ ---
        ...
        # --- KONIEC NOVÉHO FORMÁTOVANIA HLÁSENÍ ---
    ...
    def share_current_faults(self):
        ...
        if filtered_hlasenia:
            content += "HLÁSENIA PORÚCH:\n"
            for hlasenie in filtered_hlasenia:
                hlasenie_time_str = datetime.strptime(hlasenie['datum_cas'], '%Y-%m-%d %H:%M:%S').strftime('%H:%M:%S')
                duration = hlasenie.get('trvanie_minuty', 0)
                content += f"- [{hlasenie_time_str}] OP: {hlasenie['op']} | Porucha: {hlasenie['porucha']} | Trvanie: {duration} min\n"
        else:
             content += "ŽIADNE HLÁSENIA PORÚCH ZAZNAMENANÉ.\n"
            ...
            for panel_key, panel_data in sorted_panels:
                ...
                if panel_end_time < today: # Ak hodina už skončila
                    retro_button = MDButton(
                        ...
                        #text pridať spätné hlasenie zmenšení na +
                        # icon="plus", # Odstránený chybný parameter '. Must be one of: ['elevated', 'filled', 'tonal', 'outlined', 'text']
                        style="text",
                        size_hint_x=1
                    )
                    retro_button.icon = "plus" # <-- Ikona nastavená ako atribút PO vytvorení
...
class SearchScreen(MDScreen):
    ...
    def search_hlasenia(self):
        ...
        total_jph = 0
        all_jph_keys = [k for k in jph_data.keys() if ':' in k]

        for key in all_jph_keys:
            try:
                dt_obj = datetime.strptime(key.split(':')[0], '%d.%m.%y_%H')
...
class JPHScreen(MDScreen):
    ...
    def load_and_display_shifts(self):
        ...
        for key, value in jph_data.items():
            try:
                if key.startswith('plan_'):
                    _, date_str, shift_type = key.split('_')
                    date_obj = datetime.strptime(date_str, '%d.%m.%y').date()
                    shift_key = (date_obj, shift_type)
                    shifts.setdefault(shift_key, {'total_made': 0, 'plan_jph': 0, 'goal_jph': 0})['plan_jph'] = value
                elif key.startswith('goal_'):
                    _, date_str, shift_type = key.split('_')
                    date_obj = datetime.strptime(date_str, '%d.%m.%y').date()
                    shift_key = (date_obj, shift_type)
                    shifts.setdefault(shift_key, {'total_made': 0, 'plan_jph': 0, 'goal_jph': 0})['goal_jph'] = value
                elif ':' in key:
                    dt_obj = datetime.strptime(key.split(':')[0], '%d.%m.%y_%H')
                    shift_date, shift_type = self.get_shift_info_from_datetime(dt_obj)
                    shift_key = (shift_date, shift_type)
                    shifts.setdefault(shift_key, {'total_made': 0, 'plan_jph': 0, 'goal_jph': 0})['total_made'] += value
        ...
        if self.start_date or self.end_date:
            start = self.start_date if self.start_date else date.min
            end = self.end_date if self.end_date else date.max
        ...
        for k, v in display_data.items():
            ...
            if shift_type_char == 'D' and "Denná" in self.selected_shifts_filter:
                filtered_display_data[k] = v
            elif shift_type_char == 'N' and "Nočná" in self.selected_shifts_filter:
                filtered_display_data[k] = v

        display_data = filtered_display_data

        total_shifts = len(display_data)
        total_pieces = sum(v['total_made'] for v in display_data.values())
        total_hours = total_shifts * 12

        avg_per_shift = (total_pieces / total_shifts) if total_shifts > 0 else 0.0
        avg_per_hour = (total_pieces / total_hours) if total_hours > 0 else 0.0

        self.ids.summary_total_shifts.text = f"Počet smien: {total_shifts}"
        self.ids.summary_total_pieces.text = f"Celkovo kusov: {total_pieces} ks"
        self.ids.summary_avg_per_shift.text = f"Priemer / smena (12h): {avg_per_shift:.2f} ks"
        self.ids.summary_avg_per_hour.text = f"Priemer / hodina: {avg_per_hour:.2f} ks"

        summary_card = self.ids.summary_card
        if total_shifts > 0:
            if summary_card.height == 0:
                Animation(
                    height=summary_card.minimum_height,
                    opacity=1,
                    duration=0.2,
                    t="out_quad"
                ).start(summary_card)
        ...
        sorted_keys = sorted(display_data.keys(), reverse=True)

        if not sorted_keys:
            self.ids.shift_list.add_widget(
                MDListItem(MDListItemHeadlineText(text="Pre daný filter sa nenašli žiadne dáta."))
            )
            return

        app = MDApp.get_running_app()
        for key in sorted_keys:
            date_obj, shift_type = key
            data = display_data[key]
            total_made = data['total_made']
            plan_jph = data.get('goal_jph') if data.get('goal_jph', 0) > 0 else data.get('plan_jph', 0)

            diff_color = app.theme_cls.surfaceContainerColor
            plan_total = 0
            difference = total_made

            if plan_jph > 0:
                plan_total = plan_jph * 12
                difference = total_made - plan_total
                if difference >= 0:
                    diff_color = get_color_from_hex("#4CAF5040")
                else:
                    diff_color = app.theme_cls.errorContainerColor

            plan_total_display = plan_jph * 12
            difference_display = total_made - plan_total_display
            diff_str = f"+{difference_display}" if difference_display >= 0 else str(difference_display)

            headline = f"{date_obj.strftime('%d.%m.%Y')} | Smena: {shift_type}"
            supporting = f"Spravené: {total_made} ks | Cieľ: {plan_total_display} ks ({plan_jph} JPH)"
            ...
            list_item = MDListItem(
                ...
                theme_bg_color="Custom",
                md_bg_color=diff_color,
                radius=[12]*4,
                size_hint_y=None, height=dp(100)
            )
            list_item.add_widget(
                TrailingPressedIconButton(icon="delete-outline", on_release=lambda _, k=key: self.confirm_delete_shift(k))
            )
            self.ids.shift_list.add_widget(list_item)
